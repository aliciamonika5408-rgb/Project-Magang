...
function vite_assets(): array
{
    ...
    if (is_readable($manifestPath)) {
        ...
        if (is_array($manifest)) {
            // Gunakan asset() agar path benar untuk XAMPP subfolder maupun root
...
require_once dirname(__DIR__, 2) . '/native/db.php';
...
try {
    // Ambil other_services dan cast ke object agar kompatibel dengan sisa kode template
    $otherServicesRaw = db_select('SELECT * FROM other_services ORDER BY created_at DESC');
    $otherServices = array_map(fn($item) => (object) $item, $otherServicesRaw);

    // Ambil projects dan cast ke object agar kompatibel dengan sisa kode template
    $projectsRaw = db_select('SELECT * FROM projects ORDER BY created_at DESC LIMIT 6');
    $projects = array_map(fn($item) => (object) $item, $projectsRaw);

    // Ambil company_settings
            ...
            <div class="row g-0">
                <div class="col-6 col-md-3 reveal" style="transition-delay:0.1s;">
                    <div class="stat-premium-card">
                        <div class="stat-premium-number stat-number" data-target="<?= e($companySettings['years_experience'] ?? '12') ?>" data-suffix="+">0</div>
                        <div class="stat-premium-label">Tahun Pengalaman</div>
                    </div>
                </div>
                <div class="col-6 col-md-3 reveal" style="transition-delay:0.2s;">
                    <div class="stat-premium-card">
                        <div class="stat-premium-number stat-number" data-target="<?= e($companySettings['projects_completed'] ?? '1250') ?>" data-suffix="+">0</div>
            ...
            <div class="col-lg-6 reveal reveal-left">
                <div class="position-relative overflow-hidden rounded-4 shadow-lg h-100" style="min-height: 420px;">
                    ...
                    <div class="position-absolute bottom-0 start-0 bg-danger text-white p-4 rounded-4 m-3 shadow-lg float-effect" style="z-index: 3;">
                        <h4 class="fw-bold mb-0"><?= e($companySettings['years_experience'] ?? '12') ?>+ Tahun Pengalaman</h4>
