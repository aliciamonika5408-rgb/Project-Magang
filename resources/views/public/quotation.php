<?php
/**
 * Konversi Native PHP dari resources/views/public/quotation.blade.php
 * 
 * Mempertahankan desain, tampilan, HTML, CSS, JavaScript, form, progress bar, animasi, dan fitur 100% presisi.
...
if (!function_exists('route_is')) {
    function route_is(string $pattern): bool
    {
        ...
        if (str_contains($pattern, 'quotation')) {
            return str_contains($path, 'quotation');
        }
        return false;
    }
}
...
// ---------------------------------------------------------------------------
// Process Request Quotation Submission
// ---------------------------------------------------------------------------
...
// Variables for sticky form inputs (re-displaying user data safely)
$name         = '';
$companyName  = '';
$email        = '';
$whatsapp     = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $buildingArea = trim($_POST['building_area'] ?? '');
    ...
    if ($projectType === '') {
        $errors[] = 'Jenis Proyek wajib dipilih.';
    }
    if ($location === '') {
        $errors[] = 'Lokasi Proyek wajib diisi.';
    }

    // --- Upload file attachment (opsional) ---
    $filePath = null;
    if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] !== UPLOAD_ERR_NO_FILE) {
        if ($_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($fileSize > 10 * 1024 * 1024) {
                $errors[] = 'Ukuran berkas attachment maksimal 10MB.';
            }

            // Validasi ekstensi yang diizinkan
            $allowedExts = ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png', 'zip', 'rar'];
            if (!in_array($fileExt, $allowedExts, true)) {
                $errors[] = 'Format berkas tidak didukung. Harap unggah berkas PDF, Word, JPG, PNG, ZIP, atau RAR.';
            }

            if (empty($errors)) {
                $safeName   = time() . '_' . bin2hex(random_bytes(4)) . '.' . $fileExt;
                $uploadDir1 = dirname(__DIR__, 3) . '/storage/app/public/quotations';
                $uploadDir2 = dirname(__DIR__, 3) . '/public/storage/quotations';
                ...
                $destPath1 = $uploadDir1 . '/' . $safeName;
                $destPath2 = $uploadDir2 . '/' . $safeName;

                if (move_uploaded_file($tmpName, $destPath1)) {
                    @copy($destPath1, $destPath2);
                    $filePath = 'quotations/' . $safeName;
                } else {
                    $errors[] = 'Gagal mengunggah berkas attachment. Silakan coba lagi.';
                }
            }
        } else {
            $errors[] = 'Terjadi kesalahan saat mengunggah berkas. Kode error: ' . (int) $_FILES['attachment']['error'];
        }
    }
    ...
    if (!empty($errors)) {
        ...
        try {
            $now = db_now();
            db_insert_row('request_quotations', [
                'name'          => $name,
                ...
                'location'      => $location,
                'building_area' => $buildingArea !== '' ? $buildingArea : null,
            ...
            $formSuccess = true;
            $formMessage = 'Request quotation Anda berhasil dikirim! Tim kami akan segera menghubungi Anda melalui email atau WhatsApp.';

            // Reset data input jika sukses
            $name         = '';
            $companyName  = '';
            $email        = '';
            $whatsapp     = '';
            ...
            error_log('Quotation DB Insert Error: ' . $e->getMessage());
            $formSuccess = false;
            $formError   = 'Terjadi kesalahan pada sistem saat menyimpan pengajuan. Silakan coba beberapa saat lagi.';
        }
...
<body>
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-10 reveal">
                        <div class="card border rounded-4 shadow-sm p-4 p-md-5 bg-white">
                            ...
                            <!-- Alert Messages for Non-AJAX Submit -->
                            <?php if ($formSuccess === true): ?>
                                <div class="alert alert-success alert-dismissible fade show rounded-3 shadow-sm border-0 mb-4" role="alert">
                                    <i class="bi bi-check-circle-fill me-2"></i><?= e($formMessage) ?>
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            <?php elseif ($formSuccess === false): ?>
                                <div class="alert alert-danger alert-dismissible fade show rounded-3 shadow-sm border-0 mb-4" role="alert">
                                    <i class="bi bi-exclamation-triangle-fill me-2"></i><?= e($formError) ?>
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            <?php endif; ?>
                            ...
                            <form id="quotationForm" action="<?= e(route('public.quotation.submit')) ?>" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row g-4">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control border-start-0 py-2" id="name" name="name" required placeholder="Masukkan nama lengkap Anda" value="<?= e($name) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control border-start-0 py-2" id="company_name" name="company_name" placeholder="Contoh: PT Bangun Sejahtera" value="<?= e($companyName) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="email" class="form-control border-start-0 py-2" id="email" name="email" required placeholder="nama@email.com" value="<?= e($email) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control border-start-0 py-2" id="whatsapp" name="whatsapp" required placeholder="Contoh: 08123456789" value="<?= e($whatsapp) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select border-start-0 py-2" id="project_type" name="project_type" required>
                                                <option value="" disabled <?= $projectType === '' ? 'selected' : '' ?>>Pilih Jenis Proyek</option>
                                                <option value="Konstruksi Gudang" <?= $projectType === 'Konstruksi Gudang' ? 'selected' : '' ?>>Konstruksi Gudang</option>
                                                <option value="Konstruksi Baja" <?= $projectType === 'Konstruksi Baja' ? 'selected' : '' ?>>Konstruksi Baja</option>
                                                <option value="Fabrikasi Baja" <?= $projectType === 'Fabrikasi Baja' ? 'selected' : '' ?>>Fabrikasi Baja</option>
                                                <option value="Steel Erection" <?= $projectType === 'Steel Erection' ? 'selected' : '' ?>>Steel Erection</option>
                                                <option value="Bangunan Industri / Pabrik" <?= $projectType === 'Bangunan Industri / Pabrik' ? 'selected' : '' ?>>Bangunan Industri / Pabrik</option>
                                                <option value="Lainnya" <?= $projectType === 'Lainnya' ? 'selected' : '' ?>>Lainnya</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control border-start-0 py-2" id="location" name="location" required placeholder="Contoh: Karawang, Jawa Barat" value="<?= e($location) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control border-start-0 py-2" id="building_area" name="building_area" placeholder="Contoh: 2000 m²" value="<?= e($buildingArea) ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select border-start-0 py-2" id="budget" name="budget">
                                                <option value="" <?= $budget === '' ? 'selected' : '' ?>>Pilih Kisaran Anggaran</option>
                                                <option value="< 500 Juta" <?= $budget === '< 500 Juta' ? 'selected' : '' ?>>&lt; 500 Juta IDR</option>
                                                <option value="500 Juta - 1 Milyar" <?= $budget === '500 Juta - 1 Milyar' ? 'selected' : '' ?>>500 Juta - 1 Milyar IDR</option>
                                                <option value="1 Milyar - 5 Milyar" <?= $budget === '1 Milyar - 5 Milyar' ? 'selected' : '' ?>>1 Milyar - 5 Milyar IDR</option>
                                                <option value="5 Milyar - 10 Milyar" <?= $budget === '5 Milyar - 10 Milyar' ? 'selected' : '' ?>>5 Milyar - 10 Milyar IDR</option>
                                                <option value="> 10 Milyar" <?= $budget === '> 10 Milyar' ? 'selected' : '' ?>>&gt; 10 Milyar IDR</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea class="form-control py-3" id="description" name="description" rows="5" placeholder="Tuliskan spesifikasi bentang tiang, tinggi bangunan, plat lantai, dinding, kelengkapan atap, dsb..."><?= e($description) ?></textarea>
                                    </div>
    ...
    <script>
        ...
        // Quotation Form AJAX & Progress Handler
                ...
                xhr.addEventListener('load', () => {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            ...
                            showToast('Request quotation Anda telah terkirim.', true);
                            form.reset();
                        }
                    } else {
