<?php
/**
 * Konversi Native PHP dari resources/views/public/projects/detail.blade.php
 * 
...
if (!function_exists('route_is')) {
    function route_is(string $pattern): bool
    {
        ...
        if (str_contains($pattern, 'projects')) {
...
// ---------------------------------------------------------------------------
// Fetch Data (Project & Related Projects)
// ---------------------------------------------------------------------------
...
if (!isset($project)) {
    ...
    if ($pdo instanceof PDO && !empty($slug)) {
        try {
            $stmt = $pdo->prepare('SELECT * FROM projects WHERE slug = ? OR id = ? LIMIT 1');
            $stmt->execute([$slug, $slug]);
            $projectData = $stmt->fetch();
            if ($projectData) {
                // Fetch gallery images
                $imgStmt = $pdo->prepare('SELECT * FROM project_images WHERE project_id = ?');
                $imgStmt->execute([$projectData->id]);
                $galleryImages = $imgStmt->fetchAll();
                
                $projectData->images = $galleryImages;
                $project = $projectData;
                
                // Fetch related projects
                $relStmt = $pdo->prepare('SELECT * FROM projects WHERE category = ? AND id != ? LIMIT 3');
                $relStmt->execute([$projectData->category, $projectData->id]);
    ...
    if (empty($project)) {
        // Sample fallback project data for direct native PHP preview
        $project = (object) [
            'id' => 1,
            'title' => 'Konstruksi Mezzanine Industri – PT Kosmetika Global Indonesia',
            'slug' => 'konstruksi-mezzanine-kosmetika',
            'category' => 'Mezzanine',
            'location' => 'Jl. Rungkut Industri III No. 9, Surabaya',
            'year' => 2024,
            'client_name' => 'PT Kosmetika Global Indonesia',
            'budget' => 'Rp 850.000.000',
            'description' => "Pembangunan struktur mezzanine baja heavy-duty untuk perluasan kapasitas ruang operasional pabrik kosmetik tanpa mengganggu alur kerja eksisting.\n\nPekerjaan mencakup perencanaan struktur teknik (engineering design), fabrikasi kolom WF 250 & balok induk WF 200 di workshop mandiri, hingga ereksi rangka di lokasi proyek berstandar K3 ketat.",
            'image' => 'images/konstruksi-mezzanine-kosmetika.jpg',
            'images' => [
                (object) ['image_path' => 'images/konstruksi-mezzanine-hore.jpg'],
                (object) ['image_path' => 'images/pembangunan-gedung-telkom.jpg'],
                (object) ['image_path' => 'images/gudang-pabrik.jpg']
            ]
        ];
    }
}

if (!isset($relatedProjects) || !is_array($relatedProjects)) {
    $relatedProjects = [
        (object) [
            'id' => 2,
            'title' => 'Konstruksi Mezzanine Logistik – PT Hore Indonesia Sehat',
            'slug' => 'konstruksi-mezzanine-hore',
            'category' => 'Mezzanine',
            'location' => 'Kawasan Industri Driyorejo, Gresik',
            'image' => 'images/konstruksi-mezzanine-hore.jpg'
        ],
        (object) [
            'id' => 3,
            'title' => 'Gedung Kantor Rangka Baja – PT Telkom Indonesia',
            'slug' => 'pembangunan-gedung-telkom',
            'category' => 'Gedung',
            'location' => 'Margorejo Indah, Surabaya',
            'image' => 'images/pembangunan-gedung-telkom.jpg'
        ]
    ];
}

$isObj = is_object($project);
$pTitle = $isObj ? $project->title : ($project['title'] ?? '');
$pImage = $isObj ? $project->image : ($project['image'] ?? '');
$pDescription = $isObj ? $project->description : ($project['description'] ?? '');
$pCategory = $isObj ? $project->category : ($project['category'] ?? '');
$pLocation = $isObj ? $project->location : ($project['location'] ?? '');
$pClientName = $isObj ? ($project->client_name ?? null) : ($project['client_name'] ?? null);
$pBudget = $isObj ? ($project->budget ?? null) : ($project['budget'] ?? null);
$pImages = $isObj ? ($project->images ?? []) : ($project['images'] ?? []);

$primaryImgUrl = $pImage 
    ? (str_starts_with($pImage, 'http') ? $pImage : (str_starts_with($pImage, 'images/') ? asset($pImage) : asset('storage/' . $pImage)))
            ...
            <div class="container">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="<?= e(route('public.projects.index')) ?>" class="text-white-50 text-decoration-none">Projects</a></li>
                        <li class="breadcrumb-item active text-white" aria-current="page"><?= e($pTitle) ?></li>
                    </ol>
                </nav>
                <h1 class="display-5 fw-bold text-white mb-0 mt-3"><?= e($pTitle) ?></h1>
            </div>
        </div>
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-8 reveal reveal-left">
                        ...
                        <div class="rounded-4 overflow-hidden mb-4 shadow-sm" style="max-height: 480px;">
                            <img src="<?= e($primaryImgUrl) ?>" class="w-100 h-100 object-fit-cover" alt="<?= e($pTitle) ?>">
                        </div>

                        <h3 class="fw-bold text-navy mb-3">Rincian Proyek</h3>
                        <div class="text-muted leading-relaxed mb-5">
                            <?= nl2br(e($pDescription)) ?>
                        </div>

                        <!-- Secondary Gallery Images -->
                        <?php if (!empty($pImages) && count($pImages) > 0): ?>
                            <h3 class="fw-bold text-navy mb-4">Galeri Dokumentasi Lapangan</h3>
                            <div class="row g-3">
                                <?php foreach ($pImages as $galImage): ?>
                                    <?php
                                    $gObj = is_object($galImage);
                                    $imgPath = $gObj ? $galImage->image_path : ($galImage['image_path'] ?? '');
                                    ...
                                    <div class="col-sm-6 col-md-4">
                                        <div class="rounded-3 overflow-hidden shadow-sm position-relative border" style="height: 160px; transition: transform 0.3s;" onmouseover="this.style.transform='scale(1.03)'" onmouseout="this.style.transform='scale(1)'">
                                            <a href="<?= e($imgUrl) ?>" target="_blank">
                                                <img src="<?= e($imgUrl) ?>" class="w-100 h-100 object-fit-cover" alt="Proyek Dokumentasi">
                    ...
                    <div class="col-lg-4 reveal reveal-right" style="transition-delay: 0.2s;">
                        <div class="card border rounded-4 shadow-sm p-4 bg-light mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <strong class="text-navy fs-6"><?= e($pCategory) ?></strong>
                            </div>
                            <div class="mb-3">
                                <span class="text-muted text-xs d-block mb-1">LOKASI</span>
                                <strong class="text-navy fs-6"><i class="bi bi-geo-alt-fill text-warning me-1"></i><?= e($pLocation) ?></strong>
                            </div>
                            <?php if (!empty($pClientName)): ?>
                                <div class="mb-3">
                                    <span class="text-muted text-xs d-block mb-1">NAMA KLIEN</span>
                                    <strong class="text-navy fs-6"><?= e($pClientName) ?></strong>
                                </div>
                            <?php endif; ?>
                            <?php if (!empty($pBudget)): ?>
                                <div class="mb-0">
                                    <span class="text-muted text-xs d-block mb-1">ESTIMASI ANGGARAN</span>
                                    <strong class="text-navy fs-6"><?= e($pBudget) ?></strong>
                                </div>
                            <?php endif; ?>
                        </div>
                            ...
                            <div class="card border rounded-4 shadow-sm p-4 bg-white mb-4">
                                ...
                                <div class="list-group list-group-flush">
                                    <?php foreach ($relatedProjects as $rel): ?>
                                        <?php
                                        $rObj = is_object($rel);
                                        $rTitle = $rObj ? $rel->title : ($rel['title'] ?? '');
                                        $rSlug = $rObj ? ($rel->slug ?? $rel->id) : ($rel['slug'] ?? $rel['id'] ?? '');
                                        $rLoc = $rObj ? $rel->location : ($rel['location'] ?? '');
                                        $rImg = $rObj ? ($rel->image ?? '') : ($rel['image'] ?? '');
                                        $rImgUrl = $rImg
                                            ? (str_starts_with($rImg, 'http') ? $rImg : (str_starts_with($rImg, 'images/') ? asset($rImg) : asset('storage/' . $rImg)))
                                            : 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=150&auto=format&fit=crop';
                                        ?>
                                        <a href="<?= e(route('public.projects.detail', ['slug' => $rSlug])) ?>" class="list-group-item list-group-item-action py-3 px-0 d-flex align-items-center gap-3">
                                            <img src="<?= e($rImgUrl) ?>" class="rounded" style="width: 70px; height: 50px; object-fit: cover;" alt="<?= e($rTitle) ?>">
                                            <div>
                                                <h6 class="fw-bold text-navy mb-1 text-truncate" style="max-width: 190px;"><?= e($rTitle) ?></h6>
                                                <span class="text-muted text-xs"><?= e($rLoc) ?></span>
                                            </div>
                                        </a>
                                    <?php endforeach; ?>
                                </div>
