<?php
/**
 * resources/views/admin/projects/create.php
 * ─────────────────────────────────────────────────────────────────────────────
 * Form Tambah Proyek Baru Admin PHP Native PT Multi Power Abadi.
    ...
    if (!auth_verify_csrf($submittedToken)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $allowedExts, true)) {
                ...
                if (move_uploaded_file($tmpName, $destPublic)) {
                    ...
                    $errors[] = 'Gagal mengunggah gambar proyek.';
                }
            }
        ...
        if (empty($errors)) {
            try {
                ...
                $existingSlug = db_scalar('SELECT COUNT(*) FROM projects WHERE slug = ?', [$slug]);
                if ((int)$existingSlug > 0) {
                    $slug .= '-' . time();
                }

                $now = db_now();
                db_insert_row('projects', [
                    'title'       => $title,
                    'slug'        => $slug,
                    'category'    => $category,
                    ...
                    'location'    => $location,
                    'year'        => $year,
                    'client_name' => !empty($clientName) ? $clientName : null,
                ...
                $_SESSION['success'] = 'Proyek "' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '" berhasil ditambahkan!';
...
$pageTitle       = 'Tambah Proyek - Admin Dashboard';
$headerPageTitle = 'Tambah Proyek Baru';
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4" style="margin-left: auto;">
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <form action="<?= e(route('admin.projects.create')) ?>" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                            <div class="row g-4">
                                <!-- Title -->
                                <div class="col-md-6">
                                    <label for="title" class="form-label fw-semibold text-navy">Nama Proyek *</label>
                                    <input type="text" name="title" id="title" class="form-control" placeholder="Contoh: Gudang Logistik Bentang Lebar WF" required value="<?= e($old['title']) ?>">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="category" id="category" class="form-select" required>
                                        <option value="" disabled <?= empty($old['category']) ? 'selected' : '' ?>>Pilih Kategori</option>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="description" id="description" class="form-control" rows="5" placeholder="Tuliskan spesifikasi pengerjaan, tantangan teknik, luas bentang, dsb..." required><?= e($old['description']) ?></textarea>
                                </div>
