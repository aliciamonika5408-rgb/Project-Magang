<?php
/**
 * resources/views/admin/clients/edit.php
 * ─────────────────────────────────────────────────────────────────────────────
 * Form Edit Partner / Klien Admin PHP Native PT Multi Power Abadi.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!auth_verify_csrf($submittedToken)) {
        ...
        // Handle Optional Logo Upload (Jangan hapus file lama sebelum file baru terproses dengan aman)
        $logoPath = null;
        ...
        if (empty($errors)) {
            try {
                ...
                if ($logoPath !== null) {
                    db_execute(
                        'UPDATE clients SET name = ?, logo_path = ?, website_url = ?, updated_at = ? WHERE id = ?',
                        [$name, $logoPath, !empty($websiteUrl) ? $websiteUrl : null, $now, $clientId]
                    );
                } else {
                    db_execute(
                        'UPDATE clients SET name = ?, website_url = ?, updated_at = ? WHERE id = ?',
                        [$name, !empty($websiteUrl) ? $websiteUrl : null, $now, $clientId]
                    );
                }

                $_SESSION['success'] = 'Partner "' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '" berhasil diperbarui!';
                header('Location: ' . route('admin.clients.index'));
                exit;
            } catch (Throwable $e) {
...
$pageTitle       = 'Edit Partner - Admin Dashboard';
$headerPageTitle = 'Edit Partner: ' . $client['name'];
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4" style="margin-left: auto;">
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <form action="<?= e(route('admin.clients.edit', ['id' => $clientId])) ?>" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                            <input type="hidden" name="id" value="<?= e((string) $clientId) ?>">
                            <div class="row g-4">
                                <!-- Name -->
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    ...
                                    <?php if (!empty($client['logo_path'])): ?>
                                        <div class="mb-3">
                                            <?php
                                            $currLogo = str_starts_with($client['logo_path'], 'http') ? $client['logo_path'] : asset('storage/' . $client['logo_path']);
                                            ?>
                                            <div class="bg-light p-3 rounded text-center border d-inline-block" style="width: 120px; height: 70px;">
                                                <img src="<?= e($currLogo) ?>" class="img-fluid object-fit-contain h-100" alt="<?= e($client['name']) ?>">
                                            </div>
                                            <span class="text-xs text-muted mt-1 d-block">Logo Terpasang</span>
                                        </div>
                                    <?php endif; ?>
                                    <input type="file" name="logo" id="logo" class="form-control" accept="image/*">
                                    <span class="text-xs text-muted">Format: JPG, PNG, SVG, WEBP. Maksimal ukuran file: 5MB (Kosongkan jika tidak ingin merubah)</span>
                                </div>
