<?php
/**
 * resources/views/admin/services/edit.php
 * ─────────────────────────────────────────────────────────────────────────────
 * Form Edit Layanan Admin PHP Native PT Multi Power Abadi.
...
$service = db_find('SELECT * FROM services WHERE id = ?', [$serviceId]);
if (!$service) {
        ...
        if (empty($errors)) {
            try {
                ...
                if ($title !== $service['title']) {
                    ...
                    $existing = db_scalar('SELECT COUNT(*) FROM services WHERE slug = ? AND id != ?', [$slug, $serviceId]);
                    if ((int)$existing > 0) {
                        $slug .= '-' . time();
                    }
                }
                ...
                if ($imagePath !== null) {
                    db_execute(
                        'UPDATE services SET title = ?, slug = ?, description = ?, content = ?, icon = ?, image = ?, updated_at = ? WHERE id = ?',
                        [$title, $slug, $description, !empty($content) ? $content : null, $icon, $imagePath, $now, $serviceId]
                    );
                } else {
                    db_execute(
                        'UPDATE services SET title = ?, slug = ?, description = ?, content = ?, icon = ?, updated_at = ? WHERE id = ?',
                        [$title, $slug, $description, !empty($content) ? $content : null, $icon, $now, $serviceId]
                    );
                }

                $_SESSION['success'] = 'Layanan "' . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . '" berhasil diperbarui!';
                header('Location: ' . route('admin.services.index'));
                exit;
            } catch (Throwable $e) {
...
$pageTitle       = 'Edit Layanan - Admin Dashboard';
$headerPageTitle = 'Edit Layanan: ' . $service['title'];
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4" style="margin-left: auto;">
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    ...
                    <div class="card-body p-4">
                        <form action="<?= e(route('admin.services.edit', ['id' => $serviceId])) ?>" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                            <input type="hidden" name="id" value="<?= e((string) $serviceId) ?>">
                            <div class="row g-4">
                                <!-- Title -->
                                <div class="col-md-8">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="content" id="content" class="form-control" rows="6" placeholder="Masukkan rincian detail... HTML diperbolehkan."><?= e($old['content']) ?></textarea>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <?php if (!empty($service['image'])): ?>
                                        <div class="mb-3">
                                            <?php
                                            $currImg = str_starts_with($service['image'], 'http') ? $service['image'] : asset('storage/' . $service['image']);
                                            ?>
                                            <img src="<?= e($currImg) ?>" class="rounded shadow-sm object-fit-cover d-block" style="width: 150px; height: 100px;" alt="<?= e($service['title']) ?>">
