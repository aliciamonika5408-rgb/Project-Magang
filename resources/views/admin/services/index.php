<?php
/**
 * resources/views/admin/services/index.php
 * ─────────────────────────────────────────────────────────────────────────────
 * Halaman Daftar Layanan Utama Admin PHP Native PT Multi Power Abadi.
...
// ── Proses POST Action: Delete Service ───────────────────────────────────────
        ...
        if (!auth_verify_csrf($submittedToken)) {
            ...
            if ($serviceId > 0) {
                try {
                    ...
                    if ($service) {
                        db_execute('DELETE FROM services WHERE id = ?', [$serviceId]);
                        $_SESSION['success'] = 'Layanan "' . htmlspecialchars($service['title'], ENT_QUOTES, 'UTF-8') . '" berhasil dihapus.';
                    } else {
...
try {
    ...
    $_SESSION['error'] = 'Gagal mengambil data layanan: ' . $e->getMessage();
}

$pageTitle       = 'Kelola Layanan - Admin Dashboard';
$headerPageTitle = 'Daftar Layanan Konstruksi';
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4" style="margin-left: auto;">
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                        ...
                        <a href="<?= e(route('admin.services.create')) ?>" class="btn btn-navy text-white fw-semibold"><i class="bi bi-plus-lg me-1"></i> Tambah Layanan</a>
                    </div>
                                ...
                                <tbody>
                                    <?php if (!empty($services)): ?>
                                        <?php foreach ($services as $service): ?>
                                            ...
                                            $sSlug = $service['slug'];
                                            $sDesc = mb_strimwidth((string)($service['description'] ?? ''), 0, 80, '...');
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end pe-3">
                                                    <div class="btn-group">
                                                        ...
                                                        <form action="<?= e(route('admin.services.destroy')) ?>" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus layanan ini?');">
                                                            <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
