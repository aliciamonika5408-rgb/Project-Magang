 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Halaman Inbox Pesan Masuk Admin PHP Native PT Multi Power Abadi.
...
// ── Proses POST Action: Delete Contact ───────────────────────────────────────
        ...
        if (!auth_verify_csrf($submittedToken)) {
            ...
            if ($contactId > 0) {
                try {
                    ...
                    if ($contact) {
                        db_execute('DELETE FROM contacts WHERE id = ?', [$contactId]);
                        $_SESSION['success'] = 'Pesan kontak dari "' . htmlspecialchars($contact['name'], ENT_QUOTES, 'UTF-8') . '" berhasil dihapus.';
                    } else {
                        $_SESSION['error'] = 'Pesan kontak tidak ditemukan.';
                    }
                } catch (Throwable $e) {
                    $_SESSION['error'] = 'Gagal menghapus pesan: ' . $e->getMessage();
                }
            }
        }
...
// ── Ambil Data Contacts dari Database ────────────────────────────────────────
$contacts = [];
try {
    $contacts = db_select('SELECT * FROM contacts ORDER BY created_at DESC');
} catch (Throwable $e) {
    $_SESSION['error'] = 'Gagal mengambil data pesan kontak: ' . $e->getMessage();
}

$pageTitle       = 'Inbox Pesan - Admin Dashboard';
$headerPageTitle = 'Pesan Hubungi Kami';
                                ...
                                <tbody>
                                    <?php if (!empty($contacts)): ?>
                                        <?php foreach ($contacts as $contact): ?>
                                            ...
                                            $subject  = mb_strimwidth((string)($contact['subject'] ?? ''), 0, 50, '...');
                                            $createdAt = !empty($contact['created_at']) 
                                                ? date('d M Y, H:i', strtotime($contact['created_at'])) . ' WIB' 
                                                : '-';
                                            ?>
                                            <tr class="<?= !$isRead ? 'table-warning fw-semibold' : '' ?>">
                                                <td class="ps-3">
                                                    <strong class="text-navy d-block"><?= e($contact['name']) ?></strong>
                                                    <span class="text-xs text-muted"><?= e($contact['email']) ?></span>
                                                </td>
                                                <td><?= e($subject) ?></td>
                                                <td><?= e($contact['whatsapp'] ?? '-') ?></td>
                                                <td>
                                                    <?php if (!$isRead): ?>
                                                ...
                                                <td class="text-end pe-3">
                                                    <div class="btn-group">
                                                        <a href="<?= e(route('admin.contacts.show', ['id' => $ctId])) ?>" class="btn btn-outline-primary btn-sm me-1 rounded"><i class="bi bi-eye"></i> Baca</a>
                                                        <form action="<?= e(route('admin.contacts.destroy')) ?>" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pesan ini?');">
                                                            <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                                                            <input type="hidden" name="_action" value="DELETE">
                                                            <input type="hidden" name="id" value="<?= e((string) $ctId) ?>">
